...
export default function Exclusive(props) {
  ...
  const loadcoments = async () => {
    const comments = await db.collection('DeveloperApps').doc(props.match.params.id).collection('Reviews').orderBy('time', 'desc').get()
  ...
  const loadapps = async () => {
    try {
      const docRef = db.collection("DeveloperApps").doc(props.match.params.id);
  ...
  const Addreviews = async (e) => {
    ...
    await db.collection('DeveloperApps').doc(`${props.match.params.id}`).collection('Reviews').add({
      email: email,
      comment: comments,
      rating: rating,
  ...
  const addtocart =()=>{
    ...
    window.alert('App Added To Your Cart')
    history.push('/Cart')
  }
    ...
    <div className='container-fluid bg-light'>
      <div className='container bg-light  py-lg-5 px-lg-3 p-1'>
        ...
        <div className='container my-1  p-1 d-grid gap-2'>
          {
            app.type === "Paid App" ? <>
              {props.uid ? <>
                <a type="button" onClick={addtocart} className="btn btn-success">Buy Now For <span className="material-symbols-outlined ">
                  currency_rupee
                </span>{app.price}</a>
              </> : <>
                <a href="/users_login" className="btn btn-success">
                  Login
                </a>
              </>}
            </> : <>
        ...
        <div className="d-flex flex-row justify-content-between  my-1">
          <div className="p-2 ">
            <img className='app-img ' width="100%" height="100%" alt="logo" src={app.logo}></img></div>

          <div className="p-2 ">
            <img className='app-img ' width="100%" height="100%" alt="logo" src={app.Image}></img></div>



        </div>

        <div className='container rounded bg-white description bg-white mb-3 p-1'>
        ...
        <div className='container rounded bg-white description bg-white mb-2'>
          ...
          <div className="d-flex flex-column">
            ...
            <div className="col ">
              <p className='small_texts fs-6  mx-1'>Develper/Company:{app.developer}</p>
            </div>
          </div>
        </div>
 ...
 */}
          ...
          {Allreviews.map((item, index) => (
            <div key={index} className="d-flex flex-row rounded bg-white justify-content-around  mb-1">
              <div className="pt-4 ">
                <p className='small_texts  text-left '>{item.email.slice(0, 3)}...</p>
              </div>
              <div className="pt-4 ">
